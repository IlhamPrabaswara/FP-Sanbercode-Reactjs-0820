...
const GameEdit = () => {
    ...
    useEffect(() => {
        if (input === "") {
            axios.get(`https://backendexample.sanbersy.com/api/data-game/${id}`, { headers: { "Authorization": `Bearer ${user.token}` } })
                .then(
                    res => {
                        setInput({
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        axios
            .put(`https://backendexample.sanbersy.com/api/data-game/${id}`, input, { headers: { "Authorization": `Bearer ${user.token}` } })
            .then(res => {
                console.log(res)
                history.goBack()
            })
