...
const MovieEdit = () => {
    ...
    useEffect(() => {
        if (input === "") {
            axios.get(`https://backendexample.sanbersy.com/api/data-movie/${id}`, { headers: { "Authorization": `Bearer ${user.token}` } })
                .then(
                    res => {
                        setInput({
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        axios
            .put(`https://backendexample.sanbersy.com/api/data-movie/${id}`, input, { headers: { "Authorization": `Bearer ${user.token}` } })
            .then(res => {
                console.log(res)
                history.goBack()
            })
            ...
            <form onSubmit={handleSubmit} id="entry">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="title" name="title" placeholder="Judul" value={input.title} onChange={handleChange} /></td>
                    </tr>
                    <br />
                    <tr>
                        <td><label htmlFor="description">Description: </label></td>
                        <td><textarea form="entry" name="description" placeholder="Deskripsi" value={input.description} onChange={handleChange} /></td>
                    </tr>
                    <tr>
                        <td><label htmlFor="review">Review: </label></td>
                        <td><textarea form="entry" name="review" placeholder="Ripiu" value={input.review} onChange={handleChange} /></td>
                    </tr>
                    <br />
                    <tr>
